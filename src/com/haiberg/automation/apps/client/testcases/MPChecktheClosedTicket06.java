...
import com.haiberg.automation.apps.client.ui.tasks.MPChecktheClosedTicketTask06;
...
import com.haiberg.automation.util.GetData;
	...
	public class MPChecktheClosedTicket06 {
		...
		MPChecktheClosedTicketTask06 mpct =new MPChecktheClosedTicketTask06();
		...
		public void setup1() throws Exception {
			...
			String Descritpinm = "MPChecktheClosedTicket06";
			String Descritpind = "MPChecktheClosedTicket06";
			String system = "FIMS";
	...
	//***************************************************************************		
			...
			/*	logger.info("A8." + "click the history field");
				...
				logger.info("F3." + "MP checks the different Meidas");
				
				logger.info("A1" + "Check Normal media status");
				result =mpct.CheckNormalMStatus();
				coreauto.assertTrue("Result", result);
				
				logger.info("A2" + "Check Normal media value");
				result =mpct.CheckNormalMValues(media1,budget1,etflight1,duedate1);
				coreauto.assertTrue("Result", result);		
				
	//************************************************************************		P2
				logger.info("A3" + "Check Regular media status");
				result =mpct.CheckRegularMStatus();
				coreauto.assertTrue("Result", result);
				
				logger.info("A4" + "Check Regular media value");
				result =mpct.CheckRegularMValues(media2,budget2);
				coreauto.assertTrue("Result", result);		
				
				
	//************************************************************************       P3
				logger.info("A5" + "Check Special media status");
				result =mpct.CheckSpecialMStatus();
				coreauto.assertTrue("Result", result);
				
				logger.info("A6" + "Check Special media value");
				result =mpct.CheckSpecialMValues(media3,etflight3,duedate3);
				coreauto.assertTrue("Result", result);	
				
				logger.info("A7" + "Check the rest-budget Field");
				result =mpbt.CheckRestBudgetField(resbudget);
				coreauto.assertTrue("Result", result);

				logger.info("A8" + "Click the save and senden button");
				result =mpt2.ClickSaveandSendButton2();
				coreauto.assertTrue("Result", result);
				
				logger.info("A9" + "Check the hint message");
				result =mpt2.CheckHintMessage("Sie haben noch keine Änderungen durchgeführt");
				coreauto.assertTrue("Result", result);
